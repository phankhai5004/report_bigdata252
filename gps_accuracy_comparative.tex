...
Table~\ref{tab:per_route} presents per-route MDR improvement percentages.

\begin{table}[H]
\centering
\caption{Per-route MDR improvement (\%)}
\label{tab:per_route}
\begin{tabular}{lrr}
\toprule
\textbf{Route} & \textbf{KF} & \textbf{MM-KF} \\
\midrule
29  & $-1.9$   & $+3.3$  \\
41  & $-4.9$   & $+5.4$  \\
57  & $-6.0$   & $+1.8$  \\
...
151 & $-4.1$   & $+40.8$ \\
\midrule
\textbf{Fleet avg.} & $-36.5$ & $\mathbf{+41.8}$ \\
\bottomrule
\end{tabular}
\end{table}
...
(Section~\ref{sec:metrics}) uses Mean Distance to Route (MDR) to measure
route adherence.
...
MDR improvement of \textbf{+41.8\%} relative to raw GPS, substantially
outperforming the standard KF.
